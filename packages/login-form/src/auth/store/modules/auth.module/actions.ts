...
import {
  getAuthPayload, saveTokensData, saveUserDataToStorage,
} from './../../../services/helpers'
import {
  authApi, domainsApi, UserFull, UsersApi, usersApi, UsersGetRequest,
} from './../../../services/api'
...
export const actions: ActionTree<AuthState, RootState> = {
  ...
  [FETCH_USER]:async ({ commit }, data: UsersGetRequest ) => {
    const response = await ( usersApi as UsersApi ).usersGet(
      data ?? getLoggedUserData(),
    )
    commit( SET_USER, response?.items[0] ?? null )
  },

  [PASS_AUTH]: async ({ commit, dispatch, rootGetters }, loginData?: LoginData ) => {

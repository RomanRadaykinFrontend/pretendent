/* tslint:disable */
/* eslint-disable */
import { KvApi, AuthApi, AdfsApi, UsersApi, DomainsApi, Configuration, } from './openapi';
...
export const initEventBusActions = (eventBus) => {
    eventBus.$on(TokenStatus.SUCCESS, (accessToken) => initAPIs(accessToken));
    eventBus.$on(TokenStatus.RESET, () => initAPIs(''));
    eventBus.$on(TokenStatus.ERROR, (message) => {
        initAPIs('');
        console.error(message);
    });
    firstInitAPI();
};
function initAPIs(accessToken) {
...
firstInitAPI();
export let kvApi = null;
export let authApi = null;
export let adfsApi = null;
export let usersApi = null;
